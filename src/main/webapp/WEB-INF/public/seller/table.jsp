...
<head>
    <meta charset="UTF-8">
    <title>Danh sách bàn - PolyCafe</title>
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex gap-2">
                            <a href="?status=&keyword=${param.keyword}"
                               class="px-4 py-2 rounded-full text-sm shadow ${empty currentStatus ? 'bg-cafe-brown text-white' : 'bg-gray-100'}">
                                Tất cả
                            </a>

                            <a href="?status=empty&keyword=${param.keyword}"
                               class="px-4 py-2 rounded-full text-sm shadow ${currentStatus == 'empty' ? 'bg-green-500 text-white' : 'bg-gray-100'}">
                                Trống
                            </a>

                            <a href="?status=using&keyword=${param.keyword}"
                               class="px-4 py-2 rounded-full text-sm shadow ${currentStatus == 'using' ? 'bg-red-500 text-white' : 'bg-gray-100'}">
                                Đang dùng
                            </a>
                        </div>
                    </div>
                        ...
                        <c:forEach var="t" items="${tables}">
                            <div class="relative">
                                ...
                                <a href="${pageContext.request.contextPath}/seller/order?tableId=${t.id}">
                                    <div class="aspect-square rounded-xl flex flex-col justify-center items-center text-center shadow-sm hover:shadow-md transition ${t.status == 'empty' ? 'bg-green-100 border border-green-300' : 'bg-red-100 border border-red-300'}">
                                        <h3 class="font-semibold text-lg mb-2">${t.name}</h3>
                                        <p class="text-sm">
                                            <c:choose>
                                                <c:when test="${t.status == 'empty'}">Trống</c:when>
                                                <c:otherwise>Đang dùng</c:otherwise>
                                            </c:choose>
                                        </p>
                                    </div>
                                </a>
