...
<head>
    <meta charset="UTF-8">
    <title>Quản lý đồ uống - PolyCafe</title>
        ...
        <div class="p-8">
            <div class="max-w-[1400px] mx-auto">
                <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-200">
                    ...
                    <h2 class="text-xl font-semibold text-gray-800 mb-6">Quản lý đồ uống</h2>
                                ...
                                <tr class="border-t hover:bg-gray-50">
                                    ...
                                    <td>
                                        <img src="${pageContext.request.contextPath}/${d.image}"
                                             class="w-14 h-14 object-cover rounded-lg mx-auto" alt="${d.name}">
                                    </td>
                                    <td class="font-medium">${d.name}</td>
                                    <td class="text-gray-600">${String.format("%,d", d.price)} ₫</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${d.active}">
                                                <span class="px-3 py-1 rounded-full bg-green-100 text-green-700 font-medium">Hoạt động</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="px-3 py-1 rounded-full bg-gray-200 text-gray-700 font-medium">Ngừng hoạt động</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td class="space-x-2">
                                        <button class="px-3 py-1 text-sm bg-blue-500 text-white rounded-lg hover:opacity-90"
                                                data-id="${d.id}"
                                                data-name="${d.name}"
                                                data-price="${d.price}"
    ...
    <div class="bg-white rounded-xl p-6 w-[400px] shadow-lg">
        ...
        <form id="form" method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input name="name" id="name" value="${oldName}" class="w-full mt-1 border px-3 py-2 rounded">
            </div>
            ...
            <div>
                ...
                <input name="price" id="price" value="${oldPrice}" class="w-full mt-1 border px-3 py-2 rounded">
            </div>

            <div>
                <label class="text-sm text-gray-600">Trạng thái</label>
                <select name="active" id="activeSelect" class="w-full mt-1 border px-3 py-2 rounded">
            ...
            <div class="flex gap-2 pt-2">
                <button type="submit" class="w-1/2 bg-cafe-brown text-white py-2 rounded-lg">Lưu</button>
                <button type="button" onclick="closeModal()" class="w-1/2 bg-gray-400 text-white py-2 rounded-lg">Hủy</button>
            </div>
        </form>
